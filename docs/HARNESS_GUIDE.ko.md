...
여기서 말하는 하네스는 AI 코딩 에이전트가 프로젝트 안에서 일할 때 따르는 공통 운영 레이어입니다.
...
현재 구성의 핵심은 다음 세 문장으로 요약할 수 있습니다.

```text
규칙(rules)은 모든 에이전트에 공통으로 적용한다.
강제(enforcement)는 도구마다 메커니즘이 다르므로 등급(Tier)으로 나눠 관리한다.
Base harness와 project harness를 분리한다.
```

여러 에이전트가 모두 같은 규칙을 참고하게 하려면 각 도구별 파일에 같은 내용을 길게 복사해두면 안 됩니다. 복사된 규칙은 시간이 지나면 조금씩 달라집니다.
...
다만 **규칙을 읽는 것과 규칙을 강제하는 것은 다릅니다.** 강제(hook, permission)는 도구마다 메커니즘이 전혀 달라 동등하게 보편화할 수 없습니다. 그래서 에이전트를 등급으로 나눠, Tier 1에만 실제 강제를 연결하고 Tier 2는 규칙 문서만 따르게 합니다(자세한 내용은 9절).
...
| 구분            | 위치                                    | 역할                                                       |
| --------------- | --------------------------------------- | ---------------------------------------------------------- |
| Base Harness    | `universal-agent-harness` 저장소        | 팀 공통 규칙, 공통 hook, 에이전트 등급 정의, template 제공 |
...
| 영역                                           | 역할                                                        |
| ---------------------------------------------- | ----------------------------------------------------------- |
| 루트 `AGENTS.md`, `CLAUDE.md`, `opencode.json` | 각 에이전트가 처음 읽는 진입점                              |
| `.agent-harness/rules/`                        | 모든 에이전트가 공유하는 실제 규칙                          |
| `.agent-harness/hooks/`                        | 포맷, 테스트, TDD 검사 같은 자동화 스크립트 (Tier 1이 호출) |
| `.claude/settings.json`                        | Claude Code(Tier 1)가 실제로 찾는 강제 설정 — 유일한 정본   |
| `templates/project/`                           | 프로젝트별 지침과 실행 설정 template                        |
| `tests/`                                       | 하네스 자체가 깨지지 않았는지 확인하는 테스트               |

이전에는 `.agent-harness/adapters/<tool>/` 아래에 도구별 설정을 두었지만, 실제 강제는 Claude(Tier 1)만 가지며 그 설정의 네이티브 위치는 `.claude/settings.json`입니다. 어댑터 사본은 순수 중복이라 제거했습니다. Tier 2 에이전트는 강제 설정이 없으므로 어댑터 디렉터리가 필요 없습니다.
...
.claude/settings.json : Tier 1 강제 설정을 직접 작성 (이 파일이 정본)
```
...
대신 `CLAUDE.md`처럼 공통 가이드를 가리키는 진입점 파일은, 원본인 `AGENT_GUIDE.md`의 경로가 바뀌면 함께 갱신해야 합니다.

현재 `tests/test_harness.py`는 각 진입점이 공통 가이드를 올바르게 가리키는지, `.claude/settings.json`이 유일한 정본으로서 `permissions`/`hooks`를 포함하는지, 제거된 에이전트의 잔여 파일이 없는지를 검사합니다.
...
공통 규칙, 공통 자동화, Tier 1 강제 설정, 프로젝트별 설정을 각각 정해진 위치에 둡니다.
...
.agent-harness/hooks/                    공통 자동화 (Tier 1이 호출)
.claude/settings.json                    Tier 1(Claude) 강제 설정 — 유일한 정본
```
...
이 파일은 hook이 기계적으로 읽는 설정입니다. 8절의 hook들이 실제로 이 값을 참고해 동작합니다.

## 8. 현재 공통 hook

> 지금 켜져 있는 hook의 전체 목록과 on/off 상태는 [`ACTIVE_RULES.ko.md`](../.agent-harness/rules/ACTIVE_RULES.ko.md)를 참고하세요.
> 아래 절에서는 각 hook이 _어떻게_ 동작하는지를 설명합니다.

이 hook들은 Tier 1(Claude Code)의 `.claude/settings.json`에서 호출됩니다. 스크립트 자체는 stdin JSON 계약만 따르므로 특정 에이전트에 종속되지 않지만, 현재 이를 실제로 연결해 강제하는 도구는 Claude뿐입니다.
...
파일 수정 이후 테스트 명령을 실행합니다.

- 프로젝트 `harness.config.json`에 `commands.test`가 설정되어 있으면 **그 명령을 우선 실행**합니다. 명령은 `shlex.split()`으로 파싱해 quoting을 보존합니다. 명시 명령의 실행 파일이 없으면(`FileNotFoundError`) 실패로 처리합니다.
- 설정이 없으면 언어별 러너를 **모두 감지해 실행**합니다. `package.json`이 있으면 `npm test --if-present`, Python 테스트 파일이 있으면 `python -m pytest`. 둘 다 있으면 둘 다 실행합니다(멀티 러너).
- `policy.test_failure`가 `strict`이면 테스트 실패 시 exit code 2로 작업을 차단하고, 기본값 `warning`이면 경고만 남깁니다.
...
탐색 범위는 다음 두 방식을 따릅니다.

- (A) 기본: 프로젝트 전체에서 이름이 일치하는 테스트 파일을 찾습니다. 하위 디렉터리에 있어도 인정합니다.
- (B) `harness.config.json`에 `paths.tests`가 설정되어 있으면 해당 경로 안에서만 탐색합니다.

`policy.tdd_guard`가 `strict`이면 대응 테스트가 없을 때 exit code 2로 차단하고, 기본값 `warning`이면 경고만 남깁니다.

## 9. 에이전트 등급(Tier)

> 각 규칙이 어떤 도구에 강제되고 어떤 도구에는 권고 수준인지는 [`ACTIVE_RULES.ko.md`](../.agent-harness/rules/ACTIVE_RULES.ko.md)의 스냅샷을, 등급 매트릭스는 [`AGENT_GUIDE.md`](../.agent-harness/rules/AGENT_GUIDE.md#지원-에이전트-및-등급)를 함께 참고하세요.

규칙은 모든 에이전트에 공통 적용되지만, 강제 수준은 등급에 따라 다릅니다.

| 도구        | 등급               | 진입점          | 현재 적용                                                      |
| ----------- | ------------------ | --------------- | -------------------------------------------------------------- |
| Claude Code | Tier 1 (완전 강제) | `CLAUDE.md`     | native import + `.claude/settings.json`의 hook/permission 강제 |
| OpenCode    | Tier 2 (규칙만)    | `opencode.json` | `instructions`로 공통 가이드 지정, 강제 없음                   |
| Codex       | Tier 2 (규칙만)    | `AGENTS.md`     | 안내 주석으로 공통 가이드 확인 유도 (주로 PR 리뷰 컨텍스트)    |

자동 강제가 연결된 도구는 Claude Code뿐입니다. 모든 에이전트에 강제 기능을 동등하게 맞추려 하지 않는 것이 의도된 설계입니다. Tier 2 에이전트 중 Tier 1으로 올릴 가치가 확인되면, 그때 해당 도구의 강제 설정을 추가하고 등급표를 갱신합니다.
...
- TDD guard가 테스트 없는 production 파일에 경고하는지 (그리고 하위 디렉터리·`paths.tests`·strict 동작)
- 대응 테스트가 있을 때 TDD guard가 경고하지 않는지
- run_tests가 `commands.test`를 실제로 실행하고, strict 정책과 실행 파일 부재를 올바르게 처리하는지
- `ACTIVE_RULES.ko.md`의 YAML 스펙 블록이 `.claude/settings.json`과 어긋나지 않는지
...
1. 자동 강제는 Claude Code(Tier 1)에만 연결되어 있습니다. Tier 2(OpenCode, Codex)는 규칙 문서만 따릅니다.
2. 테스트 감지는 Python/JS 중심이며, 다른 언어는 `commands.test`로 직접 지정해야 합니다.
3. 포맷 실패나 테스트 실패 로그가 별도로 저장되지 않습니다.
4. 언어별 프로젝트 관례가 충분히 반영되어 있지 않습니다.
5. 보안 정책이 아직 구체적이지 않습니다.
6. "테스트 파일 존재"만 확인하지, 테스트 품질은 확인하지 않습니다.
7. project harness template은 시작점이며, 프로젝트별로 반드시 조정해야 합니다.
...
Tier 2 에이전트 중 Tier 1으로 올릴 가치가 확인되면, 그 도구의 안정적인 강제 방식(hook/권한 설정)을 추가하고 등급표를 갱신합니다. 반대로 더 이상 쓰지 않는 에이전트는 진입점과 함께 제거해 범위를 좁게 유지합니다.
...
4. Tier 1(Claude) 강제 설정은 `.claude/settings.json` 한 곳에만 두고, 어댑터 사본을 만들지 않습니다.
...
9. 새 에이전트는 먼저 등급표에 등록하고, Tier 1로 올릴 때만 강제 설정을 작성합니다.
10. 위험한 권한이나 destructive command는 기본 허용하지 않습니다.
...
4. 어떤 에이전트를 Tier 1으로 강하게 자동화하고, 어떤 에이전트를 Tier 2 규칙 수준으로 둘 것인가?
