# Universal Agent Harness 상세 가이드
...
이 자동화는 현재 Claude Code adapter에서만 직접 연결되어 있습니다.
...
```text
Universal Agent Harness stop hook complete.
```
...
이 스크립트들은 특정 에이전트에 종속되지 않도록 만들어졌습니다. 현재는 Claude adapter가 호출하지만, Codex나 Gemini에서 비슷한 hook mechanism을 안정적으로 제공하면 같은 스크립트를 재사용할 수 있습니다.
...
| 항목 | 적용 대상 | 설명 |
| --- | --- | --- |
| Claude PostToolUse format | Claude | 파일 수정 후 포맷 시도 |
| Claude PostToolUse test | Claude | 파일 수정 후 테스트 시도 |
| Claude PostToolUse TDD guard | Claude | 프로덕션 파일에 테스트가 없으면 경고 |
| Claude Stop hook | Claude | 작업 종료 메시지 출력 |
| unittest self-test | 수동 실행 | 하네스 구조와 TDD guard 검사 |
...
| Codex 자동 hook | Codex | 아직 adapter placeholder |
| Gemini 자동 hook | Gemini | 아직 adapter placeholder |
...
| Codex hooks | 아직 없음 |
| Gemini hooks | 아직 없음 |
| 포맷 자동화 | Claude adapter를 통해 실행 |
| 테스트 자동화 | Claude adapter를 통해 실행 |
| TDD 경고 | Claude adapter를 통해 실행 |
| 하네스 자체 테스트 | 있음 |
| 로그 저장 | 아직 없음 |
| 언어별 adapter | 아직 기본 수준 |
| 보안 정책 | 아직 기본 수준 |
...
- Claude adapter: hooks, 권한, stop/session hook 강화
